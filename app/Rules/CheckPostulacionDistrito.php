...
class CheckPostulacionDistrito implements Rule
{
    ...
    public function passes($attribute, $value)
    {
        $check = DB::table('T_GEND_POSTULACION_DISTRITO')
            ->join('T_GENM_POSTULACION', 'T_GENM_POSTULACION.id_postulacion', '=', 'T_GEND_POSTULACION_DISTRITO.id_postulacion')
            ->where([
                ['T_GENM_POSTULACION.id_postulacion', '=', $this->id_postulacion],
                ['T_GEND_POSTULACION_DISTRITO.flg_estado', '=', true]
            ])
            ->exists();
        return $check;
    }
