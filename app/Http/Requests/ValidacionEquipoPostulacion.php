...
class ValidacionEquipoPostulacion extends FormRequest
{
    /**
    ...
    {
        return [
            'p_nro_dni' => ['required', 'max:255', Rule::unique('T_GEND_EQUIPO_POSTULACION', 'nro_dni')->where(function ($query) {
            ...
            'p_email' => ['required', 'email:rfc,dns', 'max:255', Rule::unique('T_GEND_EQUIPO_POSTULACION', 'email')->where(function ($query) {
                return $query->where('id_postulacion', '=', $this->route('id'));
            })],
