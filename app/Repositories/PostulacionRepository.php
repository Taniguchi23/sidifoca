...
class PostulacionRepository
{
    ...
    public function fases($id)
    {
        ...
        try {
            $postulacion = $this->obtener($id);
            if ($eval1 = User::where('id_usuario', '=', $postulacion->id_usu_modifica)->first()) {
                $data['fase1'] = true;
                $data['evaluador1'] = $eval1;
                $data['observacion'] = $postulacion->observacion;
            };
            if ($postulacion_admitida = $postulacion->postulacion_admitida) {
                if ($eval2 = User::where('id_usuario', '=', $postulacion_admitida->id_usu_modifica)->first()) {
                    $data['fase2'] = true;
                    $data['evaluador2'] = $eval2;
                    $data['fecha1'] = $postulacion_admitida->fec_ingreso;
                    $data['fecha2'] = $postulacion_admitida->fec_modifica;
                    $data['puntaje_total'] = $postulacion_admitida->puntaje_total;
                }
            }
            if ($postulacion_finalista = $postulacion->postulacion_finalista) {
                if ($eval3 = User::where('id_usuario', '=', $postulacion_finalista->id_usu_modifica)->first()) {
                    $data['fase3'] = true;
                    $data['evaluador3'] = $eval3;
                    $data['fecha3'] = $postulacion_finalista->fec_modifica;
                    $data['comentario'] = $postulacion_finalista->comentario;
                    $data['flg_ganador'] = $postulacion_finalista->flg_ganador;
                }
            }
            return (object)$data;
        } catch (Exception $e) {
    ...
    public function paginar($limit, $data)
    {
        $rpta = Postulacion::with([
            ...
            ->when($data['id_concurso'], function ($query) use ($data) {
                return $query->where('id_concurso', '=', $data['id_concurso']);
            })
