...
class DbCourseRepository implements CourseRepositoryInterface
{
    ...
    public function listByCourseServer(
        ...
        int $offset = 0,
        ?string $search = '',
        ?array $tipos = null,      // <-- ahora ARRAY (ej: ['quiz','work'])
        ?string $fecha = null,
        ?array $estados = null     // <-- ahora ARRAY (ej: ['Publicado','Oculto'])
    ): array {
        // 🔎 Search
        ...
        $tipos   = array_values(array_intersect((array) $tipos,   $tiposPermitidos));
        $estados = array_values(array_intersect((array) $estados, $estadosPermitidos));
    ...
    ";
        ...
        if (!empty($tipos)) {
            ...
            $whereSql .= " AND x.actividad_tipo IN ($in) ";
            array_push($whereBindings, ...$tipos);
        }

        // ESTADOS PUBLICACIÓN (IN)
        if (!empty($estados)) {
            $in = implode(',', array_fill(0, count($estados), '?'));
            $whereSql .= " AND x.estado_publicacion IN ($in) ";
            array_push($whereBindings, ...$estados);
        }

        // FECHA (día exacto o rango 'YYYY-mm-dd..YYYY-mm-dd')
        if (!empty($fecha)) {
    ...
    quiz_sin_intento AS (
      ...
      JOIN c_item_property ip
          ...
          AND ip.ref  = q.iid
          AND ip.tool = 'quiz'
      LEFT JOIN `user` u_creador ON u_creador.id = ip.insert_user_id
      CROSS JOIN alumnos a
      LEFT JOIN track_e_exercises tex
           ON tex.c_id = q.c_id
          AND tex.exe_exo_id  = q.id
    ...
    ";
        ...
        // === WHERE dinámico (igual que el otro) ===
        $whereSql = " WHERE 1=1 ";
        $whereBindings = [];
        ...
        // SQL final (sin limit ni offset)
        $sql = $sqlBase . $whereSql . $orderBy;
        $bindings = array_merge($baseBindings, $whereBindings);
