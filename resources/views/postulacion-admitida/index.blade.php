...
<div class="container">
    ...
    {{ Form::open(['route' => 'postulacion_admitida', 'autocomplete' => 'off', 'id' => 'busqueda']) }}
    {{ Form::hidden('page', 1) }}
...
<script type="text/template" id="template">
    @show ('calificar_postulacion')
    <a href="{{ route('postulacion_admitida') }}/calificar/@id" class="btn btn-primary" title="Validar"><i class="fa fa-check"></i></a>
    @endshow
    <a href="{{ route('postulacion_admitida') }}/detalles/@id" class="btn btn-default" title="Detalles"><i class="fa fa-eye"></i></a>
</script>
@endsection
...
<script nonce="{{ request()->nonce }}">
    ...
    $(function() {
        $("#id_modalidad").change(function(e) {
            var self = this;
            $.ajax({
                url: "{{ route('postulacion_admitida.listar_tipo_postulacion') }}",
                data: {
        ...
        $("#id_categoria").change(function(e) {
            var self = this;
            $.ajax({
                url: "{{ route('postulacion_admitida.listar_tema') }}",
                data: {
